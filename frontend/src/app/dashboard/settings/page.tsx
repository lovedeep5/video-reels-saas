...
import { youtubeApi, instagramApi, authApi, YouTubeStatus, InstagramStatus, AuthUser } from "@/lib/api";
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    else if (igResult === "upgrade_required") showToast("Instagram publishing is available on Pro and Business plans.", false);
  }, [searchParams]);

  useEffect(() => {
    Promise.all([
      youtubeApi.status().then((r) => setYtStatus(r.data)).catch(() => setYtStatus({ connected: false, configured: false })),
      instagramApi.status().then((r) => setIgStatus(r.data)).catch(() => setIgStatus({ configured: false, connected: false, account: null })),
  ...
  async function handleDisconnectYt() {
    ...
    try {
      ...
      setYtStatus((s) => s ? { ...s, connected: false, channel: null } : s);
  ...
  async function handleDisconnectIg() {
    ...
    try {
      ...
      setIgStatus((s) => s ? { ...s, connected: false, account: null } : s);
      showToast("Instagram disconnected.", true);
    } catch {
      showToast("Failed to disconnect. Try again.", false);
    } finally {
    ...
    <div className="max-w-2xl mx-auto py-10 px-4 space-y-6">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <div className="flex items-center gap-3 mb-4">
          <svg viewBox="0 0 24 24" className="w-7 h-7 flex-shrink-0" fill="url(#ig-gradient-settings)">
            <defs>
              <linearGradient id="ig-gradient-settings" x1="0%" y1="100%" x2="100%" y2="0%">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">Publish clips as Reels directly to Instagram</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-yellow-900/30 border border-yellow-800 rounded-lg p-4 text-sm text-yellow-300">
            Instagram integration is not configured. Contact support.
          </div>
        ) : !isPaid && !isAdmin ? (
          <div className="space-y-3">
            <div className="bg-indigo-950/50 border border-indigo-800 rounded-lg p-4">
              ...
              <p className="text-sm text-gray-400">
                Direct Instagram publishing is available on paid plans. Free users can download clips and upload manually.
              </p>
            </div>
            <a
              href="/billing"
          ...
          <div>
            <div className="flex items-center gap-3 p-4 bg-green-900/20 border border-green-800 rounded-lg mb-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-white truncate">@{igStatus.account.username}</p>
                <p className="text-xs text-gray-400">Connected Instagram account</p>
              </div>
            </div>
            <button
          ...
          <a
            ...
            className="inline-flex items-center gap-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white text-sm font-medium py-2 px-4 rounded-lg transition-colors"
          >
        ...
        {loading ? (
          ...
          <div>
            <div className="flex items-center gap-3 p-4 bg-green-900/20 border border-green-800 rounded-lg mb-4">
              <div className="w-2.5 h-2.5 rounded-full bg-green-500 flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-white truncate">{ytStatus.channel.channel_title}</p>
            ...
            <button
              ...
              className="text-sm text-red-400 hover:text-red-300 disabled:opacity-50 transition-colors"
            >
              {disconnectingYt ? "Disconnecting..." : "Disconnect YouTube"}
            </button>
          </div>
        ) : (
          <a
